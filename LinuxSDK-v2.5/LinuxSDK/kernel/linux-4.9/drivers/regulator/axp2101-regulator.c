...
#define AXP858_OVERTEMP_SHUTDOWN_FUNC	BIT(1)

#define AXP858_WARNING_LEVEL_MASK	0x03
...
static int axp20x_set_dcdc_undervoltage_shutdown(struct axp20x_dev *axp20x, int id, u32 enable)
{
	unsigned int reg = AXP858_OUTPUT_MONITOR;
	unsigned int mask;
	unsigned int val;

	switch (axp20x->variant) {
	case AXP858_ID:
		reg = AXP858_OUTPUT_MONITOR;
		mask = 1 << (id + 1);
		val = enable << (id + 1);
		break;
	...
	return regmap_update_bits(axp20x->regmap, reg, mask, val);
}
...
static int axp2101_regulator_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < nregulators; i++) {
		...
		/*
		 * Set PMU shutdown function when undervoltage.
		 */
		ret = of_property_read_u32(rdev->dev.of_node,
					"under_voltage_shutdown", &undervol);
		if (!ret) {
			if (axp20x_set_dcdc_undervoltage_shutdown(axp20x, i, undervol))
				dev_err(&pdev->dev, "Failed to set under voltage function on %s\n",
					rdev->desc->name);
		}
	...
	if (of_property_read_u32(rdev->dev.parent->of_node,
			"overtemp_value", &tempval))
		tempval = 125;

	/* Set PMU overtemp value */
	if (tempval > 135)
		regmap_update_bits(axp20x->regmap, AXP858_VREF_TEM_SET,
				AXP858_WARNING_LEVEL_MASK, 0x03);
	else if (tempval > 125)
		regmap_update_bits(axp20x->regmap, AXP858_VREF_TEM_SET,
				AXP858_WARNING_LEVEL_MASK, 0x02);
	else if (tempval > 115)
		regmap_update_bits(axp20x->regmap, AXP858_VREF_TEM_SET,
				AXP858_WARNING_LEVEL_MASK, 0x01);
	else
		regmap_update_bits(axp20x->regmap, AXP858_VREF_TEM_SET,
				AXP858_WARNING_LEVEL_MASK, 0x00);

	if (of_property_read_u32(rdev->dev.parent->of_node,
				"overtemp_shutdown", &tempshut))
		tempshut = 0;

	if (tempshut) {
		switch (axp20x->variant) {
		case AXP858_ID:
			/* Set PMU overtemp shutdown function enable */
			regmap_update_bits(axp20x->regmap, AXP858_POWER_DOWN_DIS,
					AXP858_OVERTEMP_SHUTDOWN_FUNC, 0x02);
			break;
		default:
			dev_err(&pdev->dev, "AXP variant: %ld unsupported overtemp shutdown!\n",
				axp20x->variant);
			return -EINVAL;
		}
	}
