...
static bool phy_address_broadcast_off = false;

#define GETH_MAC_ADDRESS "00:00:00:00:00:00"
static char mac_str[] = GETH_MAC_ADDRESS;
static char mac1_str[] = GETH_MAC_ADDRESS;
...
resume:
	...
	if(priv->phy_interface == PHY_INTERFACE_MODE_RGMII) {
		...
		if(phy_address_broadcast_off == true) {
			/*
...
static void geth_chip_hwaddr(struct net_device *ndev)
{
#define MD5_SIZE	16
#define CHIP_SIZE	16
	...
	{
		for (i = 0; i < ETH_ALEN; i++)
		addr[i] = result[2 * i] + 1;	/* eth1 must diff from eth0 */
	}
...
static int geth_hw_init(struct platform_device *pdev)
{
	...
	if(of_property_read_bool(np, "phy-address-broadcast-off"))
		phy_address_broadcast_off = true;
...
static int __init set_mac_addr(char *str)
{
	...
	printk("===========YTR====mac_str=%s\n",mac_str);
	return 0;
}
static int __init set_mac1_addr(char *str)
{
	char *p = str;
	if (str && strlen(str))
		memcpy(mac1_str, p, 18);
	printk("===========YTR====mac1_str=%s\n",mac1_str);
	return 0;
}
__setup("mac_addr=", set_mac_addr);
__setup("mac1_addr=", set_mac1_addr);
#endif
