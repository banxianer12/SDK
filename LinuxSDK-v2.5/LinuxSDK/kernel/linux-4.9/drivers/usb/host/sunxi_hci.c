...
atomic_t usb_set_vbus_cnt = ATOMIC_INIT(0);
atomic_t usb_resume_set_vbus_cnt = ATOMIC_INIT(0);
...
void sunxi_resume_set_vbus(struct sunxi_hci_hcd *sunxi_hci, int is_on)
{

	mutex_lock(&usb_vbus_lock);
	atomic_add(1, &usb_set_vbus_cnt);
	atomic_sub(1, &usb_resume_set_vbus_cnt);
	if (atomic_read(&usb_resume_set_vbus_cnt) == 0) {
...
void sunxi_suspend_set_vbus(struct sunxi_hci_hcd *sunxi_hci, int is_on)
{

	mutex_lock(&usb_vbus_lock);
	atomic_sub(1, &usb_set_vbus_cnt);
	atomic_add(1, &usb_resume_set_vbus_cnt);

	if ((atomic_read(&usb_set_vbus_cnt) == 0) && (suspend_condition == 1)) {
		__sunxi_set_vbus(sunxi_hci, is_on);  /* power on */
		suspend_condition = 0;
	}
...
static void sunxi_set_vbus(struct sunxi_hci_hcd *sunxi_hci, int is_on)
{
	...
	if (sunxi_hci->usbc_no == HCI0_USBC_NO) {
		if (is_on) {
			if (atomic_read(&usb_set_vbus_cnt) == 0) {
				__sunxi_set_vbus(sunxi_hci, is_on);  /* power on */
				atomic_add(1, &usb_set_vbus_cnt);
			}
		} else {
			if (atomic_read(&usb_set_vbus_cnt) == 1) {
