...
static int dma_read_fifo(struct sunxi_udc_ep *ep, struct sunxi_udc_request *req)
{
	...
	if (g_dma_debug) {
		DMSG_INFO("dr: (ep%d, 0x%p, %d, %d, %d)\n", ep->num, &(req->req),
				req->req.length, req->req.actual, left_len);
	}
...
static int sunxi_udc_confirm_rx_ready(void)
{
	...
	if (g_dma_debug) {
		DMSG_INFO("[confirm rx ready] rx_ready_flag = %d, i = %d\n",
			rx_ready_flag, i);
	}
