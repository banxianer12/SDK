...
static int gpio_sw_probe(struct platform_device *dev)
{
	...
	for (i = 0; gpio_sw_class_attrs[i].attr.name != NULL; i++)
	{
		sysfs_create_file(&sw_gpio_entry->class.dev->kobj,&gpio_sw_class_attrs[i].attr);
	}
...
static int sunxi_init_gpio_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		if (platform_device_register(gpio_sw_dev[i])) {
			printk("%s platform_device_register fail\n",
			       sw_pdata[i]->name);
			goto INIT_ERR_FREE;
		}
	}
