...
#if (defined CONFIG_SUNXI_DI && defined CONFIG_ARCH_SUN50IW9)
...
/*set fb0*/
				...
				fb->dma_buf_fd = -1;
				fb->buf.y_addr =
				*(unsigned long long *)(&(oold_frame->phy_address));
				fb->buf.cb_addr =
					*(unsigned long long *)(&(
...
/*		preview_update(new_frame, car_reverse->config.car_direct,
	...
	disp_loop:
		...
		car_reverse->thread_mask &= (~THREAD_RUN);
		if ((car_reverse->config.input_src == 0) && car_reverse->used_oview) {
...
/*
		if (oold_frame == 0)
			oold_frame = __buffer_node_alloc(
				car_reverse->config.dev, width * heigth * 2, 0);
*/
