...
static void printf_fb_parm(void)
{
	...
	for (fb_id = 0; fb_id < SUNXI_FB_MAX; fb_id++) {
		pr_warn("fb%d en:%d ch[%d]ly[%d]z[%d]"
				"fmt:%d add[%llx] \r\n", fb_id,
...
int sunxi_fb_to_display(int fb_id, int UIid, int enable)
{
	...
	(g_fbi.layer_hdl[fb_id][1] == layer_id)) {
		__wrn("fb%d is already use ui%d,"
			"no need to change.\r\n", fb_id, UIid);
		return 0;
	}
	...
	if (((chanback == 1) && (layer_idback == 0)) ||
		...
		if ((chan != chanback) || (layer_idback != layer_id)) {
			needclear = 1;
			//if this fb use ui0&ui1&ui2,close it.
		}
	}
	...
	#ifdef CONFIG_ARCH_SUN50IW9P1_ANDROID
	g_fbi.config[fb_id].info.alpha_mode = 1;
	g_fbi.config[fb_id].info.alpha_value = 0xFF;
	#else
		...
		if (sel == g_fbi.fb_mode[fb_id]) {
			...
			if (mgr && mgr->get_layer_config
				...
				if (needclear) {
					...
					__wrn("needclear fb%d UIid %d"
						" chan[%d]lid[%d]z[%d]\r\n",
						fb_id, UIid, chanback, layer_idback,
						config1.info.zorder);
					if (0 != mgr->set_layer_config(mgr, &config1, 1)) {
						__wrn("fb %d, set_layer_config"
						"(%d,%d,%d) fail\n", fb_id, sel,
						chan, layer_id);
						return -1;
					}
				}
				memcpy(&config, &g_fbi.config[fb_id],
				...
				__wrn("sunxi_fb_to_display set fb%d "
				"UIid %d ch[%d]ly[%d]z[%d]\r\n", fb_id,
					UIid, chan, layer_id, config.info.zorder);
				if (((chan == 1) && (layer_id == 0))
					|| ((chan == 2) && (layer_id == 0))
					|| ((chan == 3) && (layer_id == 0))) {
	...
	case FBIO_FREE:
		{
			...
			if ((!g_fbi.fb_enable[fb_id])
				|| (fbinfo != info)) {
				__wrn("%s, fb%d already release ? or "
				"fb_info mismatch: fbinfo= 0x%p, info= 0x%p\n",
	...
	case FBIO_ALLOC:
		{
			...
			if (g_fbi.fb_enable[fb_id]
				|| (fbinfo != info)) {
				__wrn("%s, fb%d already enable ? or "
					"fb_info mismatch: fbinfo= 0x%p, info= 0x%p\n",
					  __func__, fb_id, fbinfo, info);
				return -1;
			}
...
static int fbinfo_init(int fb_id, int dispid, struct fb_info *info)
{
	struct disp_manager *mgr = NULL;
	...
	info->fix.line_length =
		info->var.xres_virtual * 4;
	info->fix.smem_len =
		info->fix.line_length *
		info->var.yres_virtual * 2;
	info->screen_base = NULL;
	info->pseudo_palette = g_fbi.pseudo_palette[fb_id];
	...
	disp_fb_to_var(disp_init->format[dispid],
		&(info->var));

	if ((disp_init->fb_width[dispid] == 0)
		|| (disp_init->fb_height[dispid] == 0)) {
		xres =
			bsp_disp_get_screen_width_from_output_type
			(dispid,
				disp_init->output_type[dispid],
				disp_init->output_mode[dispid]);
		yres =
		...
		xres = disp_init->fb_width[dispid];
		yres = disp_init->fb_height[dispid];
	}
	...
	info->var.xres_virtual = xres;
	info->fix.line_length =
		(xres * info->var.bits_per_pixel) >> 3;
	info->fix.smem_len =
		info->fix.line_length * yres * g_disp_drv.disp_init.buffer_num[dispid];
	if (info->fix.line_length != 0)
		info->var.yres_virtual =
			info->fix.smem_len / info->fix.line_length;
	ret = fb_map_video_memory(info);
	if (ret) {
		printk("fb map video err!\r\n");
		return -ENOMEM;
	}

	mgr = g_disp_drv.mgr[dispid];
	if (mgr && mgr->device && mgr->device->get_timings) {
		mgr->device->get_timings(mgr->device, &tt);
		if (tt.pixel_clk != 0)
			info->var.pixclock =
	...
	info->var.width =
		bsp_disp_get_screen_physical_width(dispid);
	info->var.height =
		bsp_disp_get_screen_physical_height(dispid);

	return 0;
}

int bind_fb1_to_screen1(void)
{
	int fb_id, dispid;
	struct disp_layer_config config;
	struct disp_manager *mgr = NULL;
	...
	if (fb_id >= SUNXI_FB_MAX) {
		__wrn("Exceeds the maximum number of fb!\r\n");
		return 0;
	}

	if (g_fbi.fb_enable[fb_id] == 1) {
		__wrn("%s, fb%d is already requested!\n", __func__, fb_id);
		return 0;
	}
	...
	if (fbinfo_init(fb_id, dispid, info) != 0) {
		printk("fbinfo init err!\r\n");
	}
	...
	config.channel = chanid;
	config.layer_id = layerid;
	config.enable = 1;

	if ((disp_init->fb_width[dispid] == 0) ||
		(disp_init->fb_height[dispid] == 0)) {
		src_width = bsp_disp_get_screen_width_from_output_type(
			dispid, disp_init->output_type[dispid],
			disp_init->output_mode[dispid]);
		src_height = bsp_disp_get_screen_height_from_output_type(
			dispid, disp_init->output_type[dispid],
			disp_init->output_mode[dispid]);
	} else {
		src_width = disp_init->fb_width[dispid];
		src_height = disp_init->fb_height[dispid];
	}

	config.info.screen_win.width = bsp_disp_get_screen_width_from_output_type(
		dispid, disp_init->output_type[dispid], disp_init->output_mode[dispid]);
	config.info.screen_win.height = bsp_disp_get_screen_height_from_output_type(
		dispid, disp_init->output_type[dispid], disp_init->output_mode[dispid]);
	...
	config.info.fb.size[1].width = src_width;
	config.info.fb.size[1].height = src_height;
	config.info.fb.size[2].width = src_width;
	config.info.fb.size[2].height = src_height;
	config.info.fb.color_space = DISP_BT601;

	__inf(
		"fb%d: src_w: %d  src_h: %d  screen: %d , %d  info->w: %d  info->h: %d "
		"!\r\n",
		fb_id, src_width, src_height, config.info.screen_win.width,
		config.info.screen_win.height, info->var.width, info->var.height);

	if (mgr && mgr->set_layer_config) {
		if (0 != mgr->set_layer_config(mgr, &config, 1)) {
			__wrn(
				"fb %d, set_layer_config"
				"(%d,%d,%d) fail\n",
				fb_id, dispid, config.channel, config.layer_id);
		}
	}

	if (fb_id < 1) {
		g_fbi.fb_enable[fb_id] = 1;
		g_fbi.fb_mode[fb_id] = dispid;
	}
