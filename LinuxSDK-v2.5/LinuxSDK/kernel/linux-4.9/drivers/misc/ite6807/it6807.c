...
extern struct i2c_adapter *this_adap;
...
static int it6807_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (IS_ERR(it6807_3v3)) {
		pr_err("[%s]: get it6807_vcc_3v3 failed\n", __func__);
	} else {
		ret = regulator_set_voltage(it6807_3v3, 3300000, 3300000);

		if (ret) {
			pr_err("[%s]: it6807_vcc set vol failed\n", __func__);
		}

		ret = regulator_enable(it6807_3v3);
		if (ret != 0) {
			pr_err("[%s]: it6807_vcc enable failed!\n", __func__);
...
{
	if (it6807_fsm_task) {
		printk("suspend,,,it6807_fsm_task is not exit,now need exit\n");
...
static int it6807_pm_resume(struct device *dev)
{
	...
	if (gpio_is_valid(it6807_config_info.reset_gpio)) {
		...
		if (!ret) {
			gpio_direction_output(it6807_config_info.reset_gpio, 1);
			gpio_set_value(it6807_config_info.reset_gpio, 0);
			mdelay(40);
	...
	it6807_fsm_task = kthread_create(it6807_fsm_thread, NULL, "it6807_fsm_thread");
	if (it6807_fsm_task) {
		printk("it6807_fsm_task is not exit\n");
		wake_up_process(it6807_fsm_task);
	}
	return 0;
}
