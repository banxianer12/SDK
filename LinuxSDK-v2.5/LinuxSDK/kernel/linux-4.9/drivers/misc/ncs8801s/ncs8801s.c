...
static int ncs8801s_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ncs8801s_config_info.nsc8801s_power_ldo = regulator_get(&client->dev, "ncs8801s");
	if (client->addr == 0x70) {
...
{
	if (regulator_is_enabled(ncs8801s_config_info.nsc8801s_power_ldo)) {
		regulator_disable(ncs8801s_config_info.nsc8801s_power_ldo);
	}
...
static int ncs8801s_pm_resume(struct device *dev)
{
	...
	if (!IS_ERR(ncs8801s_config_info.nsc8801s_power_ldo)) {
		printinfo("resume func set nsc8801s_power_ldo .\n");
		regulator_set_voltage(ncs8801s_config_info.nsc8801s_power_ldo, (int)(ncs8801s_config_info.nsc8801s_power_vol)*1000, (int)(ncs8801s_config_info.nsc8801s_power_vol)*1000);
		if (regulator_enable(ncs8801s_config_info.nsc8801s_power_ldo) != 0) {
			pr_err("%s: enable nsc8801s_power_ldo error!\n", __func__);
			return -1;
		}
	}
...
static int script_ncs8801s_gpio_init(void)
{
	...
	ret = of_property_read_u32(np, "nsc8801s_power_vol", &data->nsc8801s_power_vol);
	if (ret) {
		pr_err("get ncs8801s_power_vol is fail, %d\n", ret);
		return -1;
	}
	ncs8801s_config_info.nsc8801s_power_vol = data->nsc8801s_power_vol;

	data->reset_gpio.gpio = of_get_named_gpio_flags(np, "ncs8801s_two_reset", 0, (enum of_gpio_flags *)(&(data->reset_gpio)));

	data->nsc8801s_power_ldo = regulator_get(data->dev, "ncs8801s");
	if (!IS_ERR(data->nsc8801s_power_ldo)) {
		printinfo("set nsc8801s_power_ldo .\n");
		regulator_set_voltage(data->nsc8801s_power_ldo, (int)(data->nsc8801s_power_vol)*1000, (int)(data->nsc8801s_power_vol)*1000);
		if (regulator_enable(data->nsc8801s_power_ldo) != 0) {
			pr_err("%s: enable nsc8801s_power_ldo error!\n", __func__);
			goto devicetree_err;
		}
...
devicetree_err:
	if (regulator_is_enabled(data->nsc8801s_power_ldo)) {
		regulator_disable(data->nsc8801s_power_ldo);
	}

	if (data->nsc8801s_power_ldo) {
		regulator_put(data->nsc8801s_power_ldo);
		data->nsc8801s_power_ldo = NULL;
	}
...
static void __exit ncs8801s_exit(void)
{
	...
	if (ncs8801s_config_info.nsc8801s_power_ldo) {
		regulator_put(ncs8801s_config_info.nsc8801s_power_ldo);
		ncs8801s_config_info.nsc8801s_power_ldo = NULL;
	}
