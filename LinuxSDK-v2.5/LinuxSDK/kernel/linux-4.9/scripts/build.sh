...
do_init_to_dts()
{
	...
	local DTC_DEP_BOARD2=${DTC_SRC_PATH}/.board-${LICHEE_OUTPUT_CONFIGS}.dtb.d.dtc.tmp
	local DTC_SRC_BOARD2=${DTC_SRC_PATH}/.board-${LICHEE_OUTPUT_CONFIGS}.dtb.dts.tmp
	...
	if [ "${LICHEE_OUTPUT_CONFIGS}" == "tft-lcd" ] ; then
		cp -f ${LICHEE_BOARD_CONFIG_DIR}/sys_config_tft_lcd.fex ${LICHEE_BOARD_CONFIG_DIR}/sys_config.fex
	else
		if [ "${LICHEE_OUTPUT_CONFIGS}" == "lvds-lcd" ] ; then
			cp -f ${LICHEE_BOARD_CONFIG_DIR}/sys_config_lvds_lcd.fex ${LICHEE_BOARD_CONFIG_DIR}/sys_config.fex
		else
			cp -f ${LICHEE_BOARD_CONFIG_DIR}/sys_config_${LICHEE_OUTPUT_CONFIGS}.fex ${LICHEE_BOARD_CONFIG_DIR}/sys_config.fex
		fi
	fi
	...
	if [ -f $DTC_DEP_BOARD2 ]; then
		printf "Script_to_dts: use board special dts file board-${LICHEE_OUTPUT_CONFIGS}.dts\n"
		DTC_DEP_FILE=${DTC_DEP_BOARD2}
		DTC_SRC_FILE=${DTC_SRC_BOARD2}
	elif [ -f $DTC_DEP_BOARD1 ]; then
...
build_kernel()
{
    ...
    if [ ! -f .config ] ; then
		if [ "x${LICHEE_FLASH}" = "xraw_nand_ubi" ]; then
			printf "\n\033[0;31;1mUsing ubi config \
				    sun50iw9p1_ubifs_defconfig ...\033[0m\n\n"
			${MAKE} ARCH=${ARCH} defconfig \
				KBUILD_DEFCONFIG=sun50iw9p1smp_ubifs_defconfig
		else
			printf "\n\033[0;31;1mUsing default config\
				${LICHEE_KERN_DEFCONF_ABSOLUTE} ...\033[0m\n\n"
			${MAKE} ARCH=${ARCH} defconfig \
			    KBUILD_DEFCONFIG=${LICHEE_KERN_DEFCONF_RELATIVE}
		fi
    fi
	...
	local tmp_rootfs="rootfs.cpio.gz"
	local tmp_rootfs_32bit="rootfs_32bit.cpio.gz"
    else
	local tmp_rootfs="rootfs_recovery.cpio.gz"
	local tmp_rootfs_32bit="rootfs_recovery_32bit.cpio.gz"
    fi

    if [ "${ARCH}" = "arm" ]; then
        cp -f ${tmp_rootfs_32bit} output/rootfs.cpio.gz
	echo "Copy ${tmp_rootfs_32bit} for ${ARCH}"
    else
		if [ "x${LICHEE_FLASH}" = "xraw_nand_ubi" ]; then
			cp -f ubirootfs.cpio.gz output/rootfs.cpio.gz
		else
			cp -f ${tmp_rootfs} output/rootfs.cpio.gz
			echo "Copy ${tmp_rootfs} for ${ARCH}"
		fi
    fi
...
build_ramfs()
{
    ...
    # for norflsah, need gzip format to save space
    ...
    cp output/${IMAGE_NAME} ${LICHEE_PLAT_OUT}
