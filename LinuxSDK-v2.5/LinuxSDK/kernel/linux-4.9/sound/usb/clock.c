			      ...
			      struct audioformat *fmt, int rate)
{
	...
	if ((err = snd_usb_ctl_msg(dev, usb_sndctrlpipe(dev, 0), UAC_SET_CUR,
				   ...
				   data, sizeof(data))) < 0) {
		/* Delete this printing to avoid it:
		   Influences the usb disconnect so that the usb can not report
		   the usb plug-out event normally to the upper app and cause
