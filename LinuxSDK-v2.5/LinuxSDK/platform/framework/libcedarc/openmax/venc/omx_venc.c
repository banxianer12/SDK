...
//for 4k recorder @30fps
    ...
    if(impl->bSaveStreamFlag)
    {
        ...
        if(impl->m_outFile == NULL)
        {
            ...
            logd("open impl->m_outFile '/data/camera/bitstream.dat'\n");
    }
    ...
    switch (eParamIndex)
    {
        ...
        case OMX_IndexParamPortDefinition:
        {
                 ...
                 impl->m_sInPortDefType.nPortIndex)
            {
                ...
                logd(" get_parameter: width = %d, height = %d",
    ...
    switch (eParamIndex)
    {
        case OMX_IndexParamPortDefinition:
        {
                     ...
                     impl->m_sOutPortDefType.nPortIndex)
            {
                ...
                logd("init_output_port: m_framerate: %d, output width %lu height %lu.",
                                        ...
                                        OMX_IN    OMX_U32                nSizeBytes)
{
    ...
    OMX_S8                        nIndex = 0x0;
    OMX_PARAM_PORTDEFINITIONTYPE* pPortDef;
    logd("allocate buffer, nPortIndex: %s, nSizeBytes: %d",
...
static inline int processThreadCommand(AwOmxVenc* impl, CdcMessage* pMsg)
{
    ...
    {
        logd("x set state command, cmd = %s, pCmdData = %s.",
                    ...
                    else
                    {
                        ...
                        if(impl->bShowFrameSizeFlag){
                            ...
                            if (((now_time-impl->mTimeStart)/1000) >= impl->mTimeOut)
                            {
                                ...
                                if (bitrate_real)
                                {
                                    logd("venc bitrate real:%d,set:%ld , \
    ...
    while (1)
    {
        ...
        if (impl->m_state == OMX_StateExecuting && impl->m_encoder)
        {
            ...
            if(impl->bOpenStatisticFlag){
                ...
                logw("MicH264Enc, VideoEncodeOneFrame time %lld.\
                     buffer by encoder %ld and by decoder %ld.",
                    (long long int)(impl->mTimeUs2-impl->mTimeUs1), (long int)impl->m_outBufferCountOwnedByEncoder,
