                                    ...
                                    OMX_BOOL bInitBufferFlag)
{
    ...
    if(bInitBufferFlag)
    {
        int ret = initBufferInfo(pCtx, pOutBufHdr, &nBufFd);
        if(ret == -1)
            return ret;
    }
    OMX_U32 num = getPortAllocSize(pCtx->pOutPort);
                            ...
                            OMX_U32 index)
{
    ...
    {
        loge("the buffer number of ACodec is not equal to FBM buffer number,"
            "please check it");
        //abort();
        ...
        if(mRet == 0)
        {
            ...
            else // start case:
            {
                pVideoPicture = SetVideoFbmBufAddress(pCtx->m_decoder,
                                                      &mVideoPicture,
                                                      nForbiddenUseFlag);
                logv("*** call SetVideoFbmBufAddress: pVideoPicture(%p), bufFd: %d",
...
static void standbyOutBufferArr(OmxAwDecoderContext *pCtx)
{
    ...
    for(i = 0; i < OMX_MAX_VIDEO_BUFFER_NUM; i++)
    {
        ...
        else if(out_buffer_status == OWNED_BY_US)
        {
            ...
            {
                logd("pCtx->m_decoder: %p, pPic[%p]",
                  pCtx->m_decoder, pCtx->mOutputBufferInfoArr[i].pVideoPicture);
                ReturnPicture(pCtx->m_decoder,
                              pCtx->mOutputBufferInfoArr[i].pVideoPicture);
                OmxTryPostSem(pCtx->mSemOutBuffer);
            }
