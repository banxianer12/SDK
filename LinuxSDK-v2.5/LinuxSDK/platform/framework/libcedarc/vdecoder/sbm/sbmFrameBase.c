...
extern s32 Avs2SbmFrameCheckBitStreamType(SbmFrame* pSbm);

extern void Avs2SbmFrameDetectOneFramePic(SbmFrame* pSbm);
...
int SbmFrameInit(SbmInterface* pSelf, SbmConfig* pSbmConfig)
{
    ...
    pSbm ->bShowLogFlag = 1;
#endif
    if(pSbm ->bShowLogFlag == 0)
       pSbm ->bShowLogFlag = CdcGetConfigParamterInt("vdecoder_sbm_printf_log", 0);
    ...
    if(pSbm->sbmInterface.bUseNewVeMemoryProgram == 1)
    {
        ...
        for(i=0; i<MAX_FRAME_BUFFER_NUM; i++)
        {
            ...
            /*pSbmFrameBuffer->pFramePic.pNaluInfoList = \
            ...
            pSbmFrameBuffer->pSbmBufferManager->pPhyFrameBufferEnd = \
                (u32)(phyAddr+nSbmFrameBufferSize-1);
            FIFOEnqueue((FiFoQueueInst**)(&pSbm->pSbmFrameBufferEmptyQueue), \
                (FiFoQueueInst*)pSbmFrameBuffer);
        }
    }
    else
    {
    ...
    else if(pSbm->sbmInterface.nType == SBM_TYPE_FRAME_AVS2)
    {
        pSbm->checkBitStreamType = Avs2SbmFrameCheckBitStreamType;
        pSbm->detectOneFramePic  = Avs2SbmFrameDetectOneFramePic;
    }
...
static int SbmFrameUpdateDebugInfo(SbmInterface* pSelf)
{
    logv("SbmFrameUpdateDebugInfo");
    SbmFrame* pSbm = (SbmFrame*)pSelf;
    ...
    int nBufSize = SbmFrameGetBufferSize(pSelf);
    int nFrameNum = SbmFrameGetStreamFrameNum(pSelf);
    int nDataSize = SbmFrameGetStreamDataSize(pSelf);
...
SbmInterface* GetSbmInterfaceFrame(int nType)
{
    ...
    pSbmFrame->sbmInterface.updateProcInfo   = SbmFrameUpdateDebugInfo;
