...
typedef struct VideoDecoderContext
{
    ...
    int                 bSaveStreamFlag;
    int                 bSavePicFlag;
    int                 nSavePicStartNum;
    int                 nSavePicToatalNum;
    const char*         pSavePicPath;
    int                 bSaveSpecialStreamFlag;
    const char*         pSaveSpecialPath;
...
static int checkDebugConfig(VideoDecoderContext* p)
{
    //* save bitstream
...
#ifdef __ANDROID__
     char val[256];
     if(p->bSaveStreamFlag == 0)
     {
        property_get("vendor.vdec.savestream", val, "0");
        p->bSaveStreamFlag = atoi(val);
      }

     if(p->bSaveSpecialStreamFlag== 0)
     {
        property_get("vendor.vdec.saveawsp", val, "0");
        p->bSaveSpecialStreamFlag= atoi(val);
      }

      property_get("vendor.vdec.savepic.startnum", val, "0");
      p->nSavePicStartNum = atoi(val);
      property_get("vendor.vdec.savepic.totalnum", val, "0");
      p->nSavePicToatalNum = atoi(val);
      if(p->nSavePicStartNum >= 0 && p->nSavePicToatalNum >= 0 && p->nSavePicToatalNum <= 1000
          && p->nSavePicStartNum < p->nSavePicToatalNum)
      {
         p->bSavePicFlag = 1;
       }
 #endif
    ...
    if(p->bSaveStreamFlag == 0)
    {
        int nSaveStreamConfig = CdcGetConfigParamterInt("vdecoder_save_bitstream", 0);
        const char* pSaveStreamPath =  CdcGetConfigParamterString("vdecoder_save_bitstream_path","null");
        if(nSaveStreamConfig == 1)
        {
            p->bSaveStreamFlag = 1;
            if(!strcmp(pSaveStreamPath,"null"))
                fpStream = fopen(fpStreamPath, "wb");
            else
                fpStream = fopen(pSaveStreamPath, "wb");
            if(fpStream)
                p->bSaveStreamFlag = 1;
        }
        logd("bSaveStreamFlag = %d, nSaveStreamConfig = %d, fpstream = %p, path = %s",
              p->bSaveStreamFlag, nSaveStreamConfig, fpStream, pSaveStreamPath);
    }
    else
    {
        fpStream = fopen(fpStreamPath, "wb");
        if(fpStream == NULL){
            p->bSaveStreamFlag = 0;
            logw("open bitstream file failed!");
        }
    }

    //* save special bitstream
    ...
    if(p->bSaveSpecialStreamFlag == 0)
    {
        ...
        p->pSaveSpecialPath = CdcGetConfigParamterString("vdecoder_save_special_bitstream_path","null");
    }
    logv("p->bSaveSpecialStreamFlag = %d, p->pSaveSpecialPath = %s",
        p->bSaveSpecialStreamFlag, p->pSaveSpecialPath);

    //* save picture
#if DEBUG_SAVE_PICTURE
    p->bSavePicFlag = 1;
    p->nSavePicStartNum = 0;
    p->nSavePicToatalNum = 10;
#endif

    if(p->bSavePicFlag == 0)
    {
        p->bSavePicFlag = CdcGetConfigParamterInt("vdecoder_save_picture_en", 0);
        p->nSavePicStartNum = CdcGetConfigParamterInt("vdecoder_save_picture_start_num", 0);
        p->nSavePicToatalNum = CdcGetConfigParamterInt("vdecoder_save_picture_total_num", 0);
        p->pSavePicPath = CdcGetConfigParamterString("vdecoder_save_picture_path","null");
    }
    logv("bSavePicFlag = %d, startNum = %d, totalNum = %d, path = %s",
        p->bSavePicFlag, p->nSavePicStartNum,
        p->nSavePicToatalNum, p->pSavePicPath);
...
int InitializeVideoDecoder(VideoDecoder* pDecoder, VideoStreamInfo* pVideoInfo,  VConfig* pVconfig)
{
    ...
    //AVS2 only support SBM_TYPE_FRAME
    if(pVideoInfo->eCodecFormat == VIDEO_CODEC_FORMAT_AVS2)
    {
    ...
    {
        FILE *fp = NULL;
        if(p->pSaveSpecialPath == NULL || !strcmp(p->pSaveSpecialPath,"null"))
        {
            //remove(SPECIAL_STREAM_FILE);  //if the awsp file exist, remove it
            fp = fopen(SPECIAL_STREAM_FILE, "ab");
        }
        else
        {
            //remove(p->pSaveSpecialPath);  //if the awsp file exist, remove it
            fp = fopen(p->pSaveSpecialPath, "ab");
        }
        if(fp == NULL)
        {
...
static void checkDynamicShowLogFlag()
{
    ...
    property_get("vendor.cedarc.debug", prop_value, "0");
...
{
    if(p->nSavePicCount >=  p->nSavePicStartNum
        && p->nSavePicCount <  (p->nSavePicStartNum + p->nSavePicToatalNum))
    {
        ...
        {
            sprintf(name, "%s_%s_%dx%d_%d.dat",
                    p->pSavePicPath, mPixelFormatMapping[i].name,
                    pPicture->nWidth, pPicture->nHeight,p->nSavePicCount);
        }
...
static int DecideStreamBufferSize(VideoDecoderContext* p)
{
    ...
    {
        nBufferSize = p->vconfig.nVbvBufferSize > nBufferSize ? p->vconfig.nVbvBufferSize : nBufferSize;
        logd("nBufferSize=%d", nBufferSize);
        return nBufferSize;
    }
