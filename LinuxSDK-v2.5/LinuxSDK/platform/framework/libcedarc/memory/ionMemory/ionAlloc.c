...
void ion_alloc_close()
{
    ...
    if (!g_alloc_context)
    {
        loge("error: cdc memory open and close dismatch!\n");
        return;
    }
...
int ion_alloc_get_debug_info(char*  pDst, int nDstBufsize)
{
    buffer_node * tmp;
    int nTotalSize = 0;
    int count = 0;
...
int ion_get_fd(void * pbuf)
{
    ...
    if (0 == flag)
    {
        loge("get fd failed, do not find virtual address: 0x%lx \n", addr_vir);
    }
