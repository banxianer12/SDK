...
void SC_hor_filter(ISP_CTX * ispctx, unsigned char *dst_buf, unsigned char *src_ptr, int src_width,
				   int dst_width, int ratio, int init_phase, int filter_type, int isLuma)
{
	int j;
...
void do_scaler(ISP_CTX * ispctx, unsigned char *psrc, unsigned char *pdst, int src_w, int src_h,
			   int dst_w, int dst_h, int fmt, int align)
{
	int i, j, *C, r;
	...
	for (i = 0; i < ispctx->SC_dst_height; i++) {
		...
		if (base_line == line_num[0]) {
			...
			//need to fill
			SC_hor_filter(ispctx, lastline, psrc + base_line * align_width, ispctx->width,
						  ispctx->SC_dst_width, ispctx->SC_luma_hor_ratio,
						  ispctx->SC_luma_hor_init_phase, 0, 1);
		}
		...
		if (base_linep1 == line_num[0]) {
			...
			SC_hor_filter(ispctx, curline, psrc + base_linep1 * align_width, ispctx->width,
						  ispctx->SC_dst_width, ispctx->SC_luma_hor_ratio,
						  ispctx->SC_luma_hor_init_phase, 0, 1);
		}
	...
	switch (fmt) {				//0-yuv420, 1-yuv422, 2-yvu420, 3-yvu422,4-yuyv422, 5-32x32,8-argb8888, 9-rgba8888, 10-abgr8888, 11-bgra8888
		case 0:
	...
	for (i = 0; i < dst_height; i++) {
		...
		if (base_line == line_num[0]) {
			...
			//need to fill
			SC_hor_filter(ispctx, lastline, CbPtr + base_line * cro_align_width, src_width,
						  dst_width, hratio, ispctx->SC_chroma_hor_init_phase, 0, 0);
		}
		...
		if (base_linep1 == line_num[0]) {
			...
			//need to fill
			SC_hor_filter(ispctx, curline, CbPtr + base_linep1 * cro_align_width, src_width,
						  dst_width, hratio, ispctx->SC_chroma_hor_init_phase, 0, 0);
		}
	...
	for (i = 0; i < dst_height; i++) {
		...
		if (base_line == line_num[0]) {
			...
			//need to fill
			SC_hor_filter(ispctx, lastline, CrPtr + base_line * cro_align_width, src_width,
						  dst_width, hratio, ispctx->SC_chroma_hor_init_phase, 0, 0);
		}
		...
		if (base_linep1 == line_num[0]) {
			...
			SC_hor_filter(ispctx, curline, CrPtr + base_linep1 * cro_align_width, src_width,
						  dst_width, hratio, ispctx->SC_chroma_hor_init_phase, 0, 0);
		}
...
int scaler(unsigned char *psrc, unsigned char *pdst, int src_w, int src_h, int dst_w, int dst_h,
		   int fmt, int align)
{
	int i;
