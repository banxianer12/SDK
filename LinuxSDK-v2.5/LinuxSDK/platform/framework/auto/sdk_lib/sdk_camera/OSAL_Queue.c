...
int OSAL_QueueUrgent(OSAL_QUEUE * queueHandle, void *data, void *outdata)
{
	...
	if (queue->first->data == NULL) {
		...
		*p = 0;
		OSAL_MutexUnlock(queue->qMutex);
		ALOGD("OSAL_QueueUrgent CNT=%d qdata=%p data=%p \r\n", queue->numElem, queue->first->data,
			   data);
		return 0;
	}
	else {

		ALOGD("xxOSAL_QueueUrgent CNT=%d qdata=%p data=%p outdata=%p \r\n", queue->numElem,
			   queue->first->data, data, outdata);
		*p = queue->first->data;
