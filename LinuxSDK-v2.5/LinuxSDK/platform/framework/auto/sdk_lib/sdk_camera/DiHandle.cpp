...
int DiHandle::DiHandleInit(int in_w,int in_h) 
{
	...
	if((mDiWidth != 0)&&(mDiWidth != in_w)){
		LOGE("mDiWidth should not be change twice\n");
		return -1;
	}
	LOGV("DiHandle::DiHandleInit w=[%d] h=[%d]",in_w,in_h);
	mDiWidth = in_w;
	mDiHeight = in_h;
	...
	for(int i = 0;i< DI_HANDLE_BUFFER;i++){
		...
		if(ret < 0){
			printf("DiBuffInit allocAlloc failed\n\n");
            return -1;
		}
...
V4L2BUF_t * DiHandle::DiProcessLoop(V4L2BUF_t * camV4l2buf, int savePicBeforeDi, int enableTimeLog)
{
	...
	//printf("Dihandle: mDiReadId=%d vir=0x%x buf->width=%d,buf->height=%d\n",
