...
public:
    ...
    virtual int decoderDataReady(AVPacket* packet)
    {
        ...
        logd("decode write[%d/%d]\n", packet->dataLen0,packet->dataLen1);
...
int singleDecoderTest()
{
    ...
    if ((inFile == NULL) )
    {
        loge("open inFile fail:%s\n",testParam.intputFile);
        return NULL;
    }
    if ((outFile == NULL))
    {
        loge("open outFile fail:%s\n",testParam.outputFile);
        return NULL;
    }
    ...
    while (decodePos < fileSize)
    {
        ...
        if (ret >= 0)
        {
            ...
            if (NULL != picBuf.handler)
            {
                ...
                if (ret < 0)
                {
                    loge("releasePicture error.\n");
                }
                logd("decodeAsync write %d [%p-%p]bytes.\n", picBuf.dataLen0, picBuf.pAddrPhy0, picBuf.pAddrPhy1);
                //usleep(50*1000);
            }
            decodePos += nalLen;
            fseek(inFile, decodePos, SEEK_SET); //go to the real decode position.
...
int main(int argc, char** argv)
{
    ...
    strcpy((char*)testParam.outputFile,        "/tmp/output/out.yuv");
