...
extern "C" int gldestory();

//#define SDKTEST_SUSPEND 1
...
static int aging_test  = 0;
static int cameraId[MAX_TEST_CAMERA] = {0};
timer_t  mTimerID;
void __timer_cb(sigval_t sig);
...
{
	ALOGD("msgType =%x-----data=%p-----%d)", msgType, user);
	...
	if ((msgType & CAMERA_MSG_DVR_NEW_FILE) == CAMERA_MSG_DVR_NEW_FILE) {
		//dvr_factory *p_dvr = (dvr_factory *) user;
		ALOGD("(msgType =CAMERA_MSG_DVR_NEW_FILE camera %d idx =%d)", p_dvr->mCameraId, ext1);
	}
...
//======Analysis argv=========
	...
	if (argc > 2) {
		...
		printf("i is %d camera num is %d argc %d\n",i,cameranums,argc);

		if(argc > 3+cameranums ){
			second = atoi(argv[3+cameranums]);
			printf("the time is %d\n",second);
			if(second > 0){
				if (createTimer(NULL, &mTimerID, __timer_cb) == 0) {
	...
	}else
	{
			...
			"arg(x+2):numbers,suspend timer.\r\n"
			"arg(x+2):t,aging test.\r\n"
			"for example:"
			"\r\n");
			...
			printf("/sdktest 1	360  99	//suspend after 99 seconds.\r\n");
			printf("/sdktest 1	360  t	//aging test.\r\n");
			printf(" ---------------sdktest-------------------------\r\n");
			exit(-1);
	}
}
...
{
	ALOGD("suspend timer \n");
	sdk_log_print(4,"[sdktest]","\t %s enter\n",__FUNCTION__);
	...
	//while(1)
	{
		for (i=0; i<cameranums;i++){
			...
			sprintf(bufname,"64,64,0,64,150,T5 SDK,64,250,ALLWINNER");
			dvrs[i]->setWaterMarkMultiple(bufname);

			rt = dvrs[i]->startPriview(vv);
			test_dvrstat[i].preview = 1;
...
int main(int argc, char *argv[])
{
	...
	cameranums = atoi(argv[1]);
	if((argc>(cameranums+2))&&argv[cameranums+2]&&(0==strcmp(argv[cameranums+2],"t"))){
		aging_test = 1;
		printf("aging_test is been set.\n");
	}
...
retry:
	...
	for(i=0;i<1;i++){
		...
		sleep(60);
		printf("aging test looping \n");
		while(aging_test == 1){
		    printf("\n\n\n aging testing...\n\n\n");
		    sleep(1);// aging test.
		}
		printf("disable watermark\n");
		#ifndef SDKTEST_SUSPEND
		dvrs[i]->disableWaterMark();
		#endif
	}
	
	printf("\n\n\n stopPriview,stopRecord,exit!\n\n\n");
	for(i=0;i<cameranums;i++){
		if(test_dvrstat[i].preview ==1){
			rt = dvrs[i]->stopPriview();
			test_dvrstat[i].preview =0;
