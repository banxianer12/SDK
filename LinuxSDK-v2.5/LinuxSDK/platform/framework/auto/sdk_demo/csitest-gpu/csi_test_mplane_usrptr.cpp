...

{
	...
	if (dev.layer_info.pixformat == TVD_PL_YUV420) {
		...
		if(V4L2_MEMORY_MMAP == memtype ){
			dev.layer_info.layer_config.info.fb.addr[0] = buf->m.planes[0].m.mem_offset;
			dev.layer_info.layer_config.info.fb.addr[1] = buf->m.planes[0].m.mem_offset + width + height;
			//dev.layer_info.layer_config.info.fb.addr[2] = buf->m.planes[2].m.mem_offset;
		}else{
			//printf("buf->m.planes[0].m.userptr=%lx\nbuffers[%d].start[0])=%lx\n", buf->m.planes[0].m.userptr, buf->index, buffers[buf->index].start[0]);

			dev.layer_info.layer_config.info.fb.addr[0] = IonVir2phy((void *)buffers[buf->index].start[0]);
			//dev.layer_info.layer_config.info.fb.addr[1] = dev.layer_info.layer_config.info.fb.addr[0] + width + height;
			//dev.layer_info.layer_config.info.fb.addr[2] = buf->m.planes[2].m.mem_offset;
		}
...
static int read_frame(int mode)
{
	...
	memcpy(gDispTab[buf.index].vir,(void *)buffers[buf.index].start[0],gDispTab[buf.index].len);
	IonFlushCache(gDispTab[buf.index].vir,gDispTab[buf.index].len );
#if 1
	//normal mode,eend camera data to gpu in 33ms
	info_aframe(&gDispTab[buf.index].dmafd);
#else
	//fast mode ,send camera data in 10ms loop
	...
	if(dispcnt >=7){
		for(int i=0;i<6;i++){
			...
			//printf("cam inteval use (%lldms)\n",gtime3/1000);
			gtime1 = gtime2;

			info_aframe(&gDispTab[i].dmafd);
			//usleep(10000);  //10ms
			usleep(10);  //10ms
		}
	}
...
static int disp_init(int width, int height, unsigned int pixformat)
{
	...
	if (dev.layer_info.pixformat == TVD_PL_YUV420)
		dev.layer_info.layer_config.info.fb.format = DISP_FORMAT_YUV420_P;  /*DISP_FORMAT_YUV420_P ---- V4L2_PIX_FMT_YUV420M*/
																			/*DISP_FORMAT_YUV420_SP_UVUV ---- V4L2_PIX_FMT_NV12*/
	else
	...
	for (n_buffers = 0; n_buffers < req.count; ++n_buffers) {
		...
		case V4L2_MEMORY_DMABUF:
			for (int j = 0; j < nplanes; j++) {
				buffers[n_buffers].length[j] = ALIGN_16B(ALIGN_16B(input_size.width)*input_size.height*3/2);
...
static int main_test(int sel, int mode)
{
	...
	for (int i = 0; i < DISP_ION_NUM; i++) {
		gDispTab[i].len = pic_size;
		gDispTab[i].vir = (void *)IonAlloc(pic_size);
		gDispTab[i].dmafd = IonVir2fd(gDispTab[i].vir);
	}
