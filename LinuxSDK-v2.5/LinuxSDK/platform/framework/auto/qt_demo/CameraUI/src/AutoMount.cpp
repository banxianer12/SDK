...
{
	printf("===RdyRecord,bRdy:%d\n",bRdy);

	dvr_factory **pDvrFac = g_ppDvr; //mod lss
	if(NULL == pDvrFac){
	...
	int iCnt = 0;
	if(bRdy){
		for(iCnt = 0; iCnt < MAX_RED_VIDEOS; iCnt++,*pDvrFac++){
			(*pDvrFac)->startRecord();
			(*pDvrFac)->SetCurRecordStat(RecordStat_NORMAL);
		}
	}else {
		for(iCnt = 0; iCnt < MAX_RED_VIDEOS; iCnt++,*pDvrFac++){
			if(RecordStat_NORMAL == (*pDvrFac)->GetCutRecortStat()){
				(*pDvrFac)->stopRecord();
				(*pDvrFac)->SetCurRecordStat(RecordStat_STOPCAR);
			}
		}
	}
