    ...
    ui(new Ui::ReverseLine_Setting)
{
    ...
    connect(this,SIGNAL(reverseLine_repaint()),g_pStaticMaindesktop,SLOT(on_reverseLine_repaint()));
...
void ReverseLine_Setting::on_pushButton_2_clicked()
{
    ...
    if(g_iRotateAngle<30){
        qDebug()<<"now rotate_angle is :"<<g_iRotateAngle;
        g_iRotateAngle+=interval;
