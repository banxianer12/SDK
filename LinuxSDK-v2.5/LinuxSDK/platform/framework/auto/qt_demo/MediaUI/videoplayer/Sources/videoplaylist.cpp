...
{
    ui->stackedWidget->setCurrentWidget(ui->udiskVideoList);
    ui->statusLabel->setText("状态：扫描u盘文件");
    ui->titleLabel->setText("u盘媒体文件");
...
void VideoPlayList::on_refreshButton_clicked()
{
    ...
    ScanVideoFileThread* scanThread = new ScanVideoFileThread(this, m_curFilePath);
...
ScanVideoFileThread::ScanVideoFileThread(QObject *parent, QString curPath)
    : QThread(parent)
{
	m_curFilePath = curPath;
    qDebug() << "ScanVideoFileThread Thread : " << QThread::currentThreadId();
}
ScanVideoFileThread::~ScanVideoFileThread()
{
    qDebug()<<"~ScanVideoFileThread";
}

void ScanVideoFileThread::run()
{
    qDebug() << "ScanVideoFileThread Run Thread : " << QThread::currentThreadId() ;
	qDebug() << "m_curFilePath: " << m_curFilePath ;
	
	QFileInfo info = QFileInfo(m_curFilePath);
	QString absPath = info.absolutePath();
	qDebug() << "absPath: " << absPath ;
    QDir* dir = new QDir(absPath); //VIDEO_USB_PATH
    if(dir == NULL){
...
void VideoPlayList::hidePlayList()
{
    ...
    animation->setEndValue(QRect(1280, 55, 320, 450));
    animation->start();
    //hide operation
    connect(animation, SIGNAL(finished()), this, SLOT(hide()));
}
void VideoPlayList::showPlayList(QString str)
{
    qDebug()<<"showPlayList";
	m_curFilePath = str;
    QPropertyAnimation *animation = new QPropertyAnimation(this, "geometry");
    ...
    animation->setEasingCurve(QEasingCurve::InQuad);
    animation->setStartValue(QRect(1280, 55, 320, 450));
