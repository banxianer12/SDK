    ...
    ui(new Ui::searchFileDialog)
{
    ...
    if(m_FileType == 0){
        ...
        m_filter << "*.mp3 *.wav *.ape *.flac *.aac *.wma *.amr *.m4a *.m4r *.ogg";
    }else if(m_FileType == 1){
        ui->titleLabel->setText("查找视频");
        m_filter << "*.divx *.3gp *.asf *.avi *.flv *.mkv *.mov *.mp4 *.mpeg *.mpg *.ra *.rm *.rmvb *.ts *.vob *.webm *.wma *.wmv *.m2ts *.3g2";
...
Description:
***************************************/
void searchFileDialog::showD(QString str)
{
    createFileDialog();
    this->show();
