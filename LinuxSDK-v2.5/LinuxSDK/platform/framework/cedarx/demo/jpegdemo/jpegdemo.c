...
#if SAVE_RGB
    FILE* outFp = fopen("/mnt/rgb.bmp", "wb");
    if(outFp != NULL)
    {
        logd("************save_bmp_rgb565\n");
...
static int JpegSaveRGBPicture(struct ScMemOpsS* memops, VideoPicture *videoPicture)
{
    VideoFrame jpegData;
    ...
    {
        loge("save picture fail !,cause mData == NULL");
        return -1;
    }
...
static void JpegSaveNormalPicture(VideoPicture *videoPicture, char *pOutUri)
{
    int nMappSize = sizeof(mPixelFormatMapping)/sizeof(PixelFormatMapping);
    int i = 0;
    for(; i < nMappSize; i++)
    {
        if(mPixelFormatMapping[i].nIndex == videoPicture->ePixelFormat)
            break;
    }
    if(i >= nMappSize)
        i = 0;
    ...
    if(pOutUri != NULL)
    {
        sprintf(pOutUri, "/mnt/pic_%s_%dx%d.yuv",mPixelFormatMapping[i].name,
...
#if SAVE_RGB
    FILE* outFp = fopen(pOutUri, "wb");
    if(outFp != NULL)
    {
        logd("saving picture, size: %d x %d, format: %d",
        videoPicture->nWidth, videoPicture->nHeight, videoPicture->ePixelFormat);

        int nSaveLen = videoPicture->nWidth * videoPicture->nHeight * 3/2;
        fwrite(videoPicture->pData0, 1, nSaveLen, outFp);
        fclose(outFp);
    }
    else
    {
        loge("saving picture failed: outFp == NULL");
    }
#endif
}

static void JpegPrintDemoUsage(void)
{
    logd("===============================");
    logd("Usage:");
    logd("the demo need only two parameters");
    logd("give an example: ./jpegdemo  /sdcard/inputpic.jpeg");
    logd("the decoded picture is located:/mnt/pic_xxx.yuv");
    logd("===============================");
}
...
int main(int argc, char** argv)
{
    ...
    pOutUri = calloc(DEMO_FILE_NAME_LEN, 1);
    if(pOutUri == NULL)
    {
        loge("pOutUri == NULL");
        return -1;
    ...
    switch (ret)
    {
        ...
        case VDECODE_RESULT_NO_FRAME_BUFFER:
        {
            ...
            if (ret>= 0)
            {
                ...
                if (videoPicture->ePixelFormat == PIXEL_FORMAT_YUV_MB32_420)
                {
                    JpegSaveRGBPicture(memops, videoPicture);
                }
                else
                {
                    JpegSaveNormalPicture(videoPicture, pOutUri);
                }
                sync();
            }
            else
