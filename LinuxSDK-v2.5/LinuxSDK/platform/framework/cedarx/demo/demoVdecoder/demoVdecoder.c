...
//    int len = strlen(value);
    ...
    switch(arg)
    {
        ...
        case SAVE_FRAME_PATH:
            ...
            {
                sscanf(value, "%s", Decoder->pSavePicPath);
...
void *displayPictureThreadFunc(void* param)
{
    ...
    while(1)
    {
        ...
        if(pPicture != NULL)
        {
            ...
            logd(" picture size[%d]: %dx%d ",pVideoDecoder->nChannel, pPicture->nWidth, pPicture->nHeight);
...
void* ChannelThread(void* param)
{
    ...
    pDec->pInputFile = NULL;
    pDec->nSavePictureNumber = 0;
    pDec->nSavePictureStartNumber = 0xffffff;
    ...
    if(pDec->argc >= 2)
    {
        for(i = 1; i < pDec->argc; i += 2)
        {
            ParseArgument(pDec, pDec->argv[i], pDec->argv[i + 1]);
        }
    }
    else
    {
...
int main(int argc, char** argv)
{
    ...
    logd("nDecoderNum = %d",nDecoderNum);
    ...
    for(i = 0; i < nDecoderNum; i++)
    {
        ...
        pDecoder[i]->argc = argc;
        pDecoder[i]->argv = argv;
