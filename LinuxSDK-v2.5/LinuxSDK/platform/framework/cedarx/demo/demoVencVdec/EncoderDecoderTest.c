...
#define ENC_DEC_MAX_NUM (5)

#define COMPUTE_ENCODER_SNR (0)
...
{
    { "-h",  "--help",    HELP,
        "Print this help, use this demo as: 'testEncDec -i 720p.yuv -o out.yuv -d 720 -s 720 -n 100 -ss 0 -sn 10 -codec_num 2'" },
    { "-i",  "--input",   INPUT,
        "Input file path" },
    ...
    { "-codec_num",  "--enc dec num",  ENC_DEC_NUM,
        "encoder and decoder thread num" },
};

static void savePicture(VideoPicture* pPicture, VdecoderContext* pVdecoderCxt)
{
    ...
    if(pVdecoderCxt->out_file_fp == NULL)
    {
        pVdecoderCxt->out_file_fp = fopen(pVdecoderCxt->out_file_path, "wb");
        if(pVdecoderCxt->out_file_fp == NULL)
        {
            logw("open file faile: %s",pVdecoderCxt->out_file_path);
            return;
        }
    }
    ...
    logd(" save picture to file: %s, size: %dx%d, top: %d, bottom: %d, left: %d, right: %d",
            (char*)pVdecoderCxt->out_file_fp, pPicture->nWidth, pPicture->nHeight,pPicture->nTopOffset,
    ...
    CdcMemFlushCache(pVdecoderCxt->videoConfig.memops, pPicture->pData0, nSaveLen);

    /* save y */
    pData = pPicture->pData0;
    nSize = nSizeY;
    fwrite(pData, 1, nSize, pVdecoderCxt->out_file_fp);
...
void ParseArgument(MainContext *pMainCxt, char *argument, char *value)
{
    encode_param_t *encode_param = &pMainCxt->vencoderCxt.encode_param;
    ARGUMENT_T arg;
    ...
    switch(arg)
    {
        ...
        case ENCODE_FORMAT:
            ...
            break;
        case OUTPUT:
            memset(pMainCxt->vdecoderCxt.out_file_path, 0, sizeof(pMainCxt->vdecoderCxt.out_file_path));
            char outFile[64] = {0};
            sscanf(value, "%63s",outFile);
            sprintf(pMainCxt->vdecoderCxt.out_file_path, "%s",outFile);
            logd(" get out file: %s ", pMainCxt->vdecoderCxt.out_file_path);
            break;
        case SRC_SIZE:
        ...
        case SAVE_FRAME_START:
            sscanf(value, "%d", &pMainCxt->vdecoderCxt.nSaveStartNum);
            break;
        case SAVE_FRAME_NUM:
            sscanf(value, "%d", &pMainCxt->vdecoderCxt.nSaveTotalNum);
            break;
            case COMPARE_FILE:
            break;
        case INVALID:
...
static int initVdecoder(MainContext* pMainCxt)
{
    ...
    #if COMPUTE_ENCODER_SNR
    pVdecoderCxt->encode_file = fopen(pVencoderCxt->encode_param.intput_file_path, "r");
...
static int copyBitstreamToDecoder(MainContext* pMainCxt)
{
    int nRet = -1;
    VencoderContext* pVencoderCxt = &pMainCxt->vencoderCxt;
    VdecoderContext* pVdecoderCxt = &pMainCxt->vdecoderCxt;
    ...
    do
    {
        ...
        {
            logv(" reqeust video stream buffer failed: nRet[%d], reqeustSize[%d], realSize[%d]",
                 nRet, nRequireSize, nBufSize + nRingBufSize);
            usleep((5*1000/pMainCxt->nEncDecNum));
        }
...
static void* VdecoderThread(void* param)
{
    ...
    while(1)
    {
        ...
        logv("[%d]decode nRet = %d, nBitstreamTotalSize = %d",
             pMainCxt->nChannel, nRet, pVdecoderCxt->nBitstreamTotalSize);

        if(pVencoderCxt->bEncoderFinishFlag == 1 && nRet == VDECODE_RESULT_NO_BITSTREAM)
        {
            bDecodeFinish = 1;
        }
        ...
        if(pVideoPic)
        {
            ...
            {
                loge("picSize of encoder and decoder not match: %d, %d;, w&h = %d, %d",
            ...
            #endif
            logv("[%d]had reqeust a picture , %d, %d, %d***",
                pMainCxt->nChannel, pVdecoderCxt->nRequestPicCount,
                pVdecoderCxt->nSaveStartNum,pVdecoderCxt->nSaveTotalNum);

            if(pVdecoderCxt->nRequestPicCount >= pVdecoderCxt->nSaveStartNum
                && pVdecoderCxt->nRequestPicCount < (pVdecoderCxt->nSaveStartNum + pVdecoderCxt->nSaveTotalNum))
                savePicture(pVideoPic, pVdecoderCxt);

            ReturnPicture(pVideoDec, pVideoPic);
            pVideoPic = NULL;
        }
        else if(bDecodeFinish == 1)
            goto OUT;
...
OUT:
    logd("exit vdecoderthread nRequestPicCount: %d\n",pVdecoderCxt->nRequestPicCount);
    pthread_exit(NULL);
    return NULL;
}


static void* EncoderThread(void* param)
{
    MainContext* pMainCxt = (MainContext*)param;
    VencoderContext* pVencoderCxt = &pMainCxt->vencoderCxt;
    VdecoderContext* pVdecoderCxt = &pMainCxt->vdecoderCxt;
    ...
    while(1)
    {
        if(nEncodeNum >= pVencoderCxt->encode_param.encode_frame_num)
            break;
        ...
        {
            fseek(pVencoderCxt->in_file, 0L, SEEK_SET);
            size1 = fread(inputBuffer.pAddrVirY, 1,
                    pBaseConfig->nInputWidth*pBaseConfig->nInputHeight, pVencoderCxt->in_file);
            size2 = fread(inputBuffer.pAddrVirC, 1,
                      pBaseConfig->nInputWidth*pBaseConfig->nInputHeight/2, pVencoderCxt->in_file);
        ...
        logv("[%d]encode frame %d use time is %lldus..\n",
            pMainCxt->nChannel, nEncodeNum,(time2-time1));
        ...
        ReturnOneAllocInputBuffer(pVencoderCxt->pVideoEnc, &inputBuffer);
         nEncodeNum++;
        //* get the bit stream and copy to decoder
        copyBitstreamToDecoder(pMainCxt);
    ...
    logd("exit encoderthread nEncodeNum: %d\n",nEncodeNum);
    pthread_exit(NULL);
    return NULL;
...
static void* ChannelThread(void* param)
{
    log_set_level(LOG_LEVEL_DEBUG);
    MainContext *pMainContext = (MainContext *)param;
    pthread_t vencoder_thread;
    pthread_t vdecoder_thread;
    VencoderContext* pVencoderCxt = &pMainContext->vencoderCxt;
    VdecoderContext* pVdecoderCxt = &pMainContext->vdecoderCxt;
    int nRet = -1;
    int i = 0;

    //set the default encode param
    encode_param_t*   pEncode_param = &pMainContext->vencoderCxt.encode_param;
    ...
    pEncode_param->maxKeyFrame = 30/*30*/;
    ...
    if(pMainContext->argc >= 2)
    {
        for(i = 1; i < (int)pMainContext->argc; i += 2)
        {
            ParseArgument(pMainContext, pMainContext->argv[i], pMainContext->argv[i + 1]);
        }
    }
    else
    {
    ...
    nRet = initVencoder(&pMainContext->vencoderCxt);
    if(nRet < 0)
    {
    ...
    nRet = initVdecoder(pMainContext);
    if(nRet < 0)
    {
        loge("*** init vdecoder failed ***");
        goto out;
    }
    pthread_create(&vdecoder_thread, NULL, VdecoderThread, (void*)(pMainContext));
    pthread_create(&vencoder_thread, NULL, EncoderThread, (void*)(pMainContext));

    pthread_join(vdecoder_thread, (void**)&nRet);
    pthread_join(vencoder_thread, (void**)&nRet);

out:

    if(pVdecoderCxt->out_file_fp)
        fclose(pVdecoderCxt->out_file_fp);
...
{
    pthread_t tchanelEncDec[ENC_DEC_MAX_NUM];
    MainContext  *pMainContext[ENC_DEC_MAX_NUM] = {NULL};
    int nRet = 0;
    int i = 0;
    int nEncDecNum = 1;
    ...
    if(nEncDecNum <= 0)
        nEncDecNum = 1;
    else if(nEncDecNum > ENC_DEC_MAX_NUM)
        nEncDecNum = ENC_DEC_MAX_NUM;

    logd("nEncDecNum = %d",nEncDecNum);

    for(i = 0; i < nEncDecNum; i++)
    {
        pMainContext[i] = calloc(sizeof(MainContext), 1);
        if(pMainContext[i] == NULL)
        {
            loge("malloc for pMainContext failed");
            goto OUT;
        }
        pMainContext[i]->argc = argc;
        pMainContext[i]->argv = argv;
        pMainContext[i]->nChannel = i;
        pMainContext[i]->nEncDecNum = nEncDecNum;
    }

    for(i = 0; i < nEncDecNum; i++)
    {
        pthread_create(&tchanelEncDec[i], NULL, ChannelThread, (void*)(pMainContext[i]));
    }

    for(i = 0; i < nEncDecNum; i++)
    {
        pthread_join(tchanelEncDec[i], (void**)&nRet);
    }
