...
void* VideoInputThread(void *param)
{
    ...
    {
        CdxIonOpen();
        p->pAddrVirY = CdxIonPalloc(sizeY);
        p->pAddrVirC = CdxIonPalloc(sizeY/2);
        printf("==== palloc demoRecoder.pAddrVirY: %p\n", p->pAddrVirY);

        videoInputBuffer.nID = 0;
        fread(p->pAddrVirY, 1, sizeY,  inputYUV);
        fread(p->pAddrVirC, 1, sizeY/2, inputYUV);
        CdxIonFlushCache(p->pAddrVirY, sizeY);
        CdxIonFlushCache(p->pAddrVirC, sizeY/2);

        videoInputBuffer.nID = 0;
        videoInputBuffer.pAddrPhyY = CdxIonVir2Phy(p->pAddrVirY);
        videoInputBuffer.pAddrPhyC = CdxIonVir2Phy(p->pAddrVirC);
    ...
    while(num<100)
    {
        ...
        else
        {
            ...
            if((int)size1 < videoInputBuffer.nLen)
            {
                logd("read error");
                videoEos = 1;
            }
    ...
    if(p->pAddrVirY)
    {
        CdxIonPfree(p->pAddrVirY);
    }
    printf("==== freee demoRecoder.pAddrVirY  end\n");

    if(p->pAddrVirC)
    {
        CdxIonPfree(p->pAddrVirC);
    }
...
int main(int argc, char *argv[])
{
    ...
    {
        printf("run failed \n");
        printf("./recoderdemo argv[1] argv[2] argv[3] argv[4]\n");
        printf(" argv[1]: video yuv data file \n");
        printf(" argv[2]: audio pcm file \n");
        printf(" argv[3]: audio encoder type (0: aac; 1: lpcm; 2:pcm; 3:mp3) \n");
        printf(" argv[4]: muxer type (0: mp4; 1: ts; 2:avi(not support now); 3:aac; 4:mp3) \n");
        printf(" argv[5]: video srcsize can be 1920x1080 or 1280*720 \n");
        printf(" argv[6]: video dstsize can be 1920x1080 or 1280*720 \n");
        return -1;
    }
    ...
    sscanf(argv[5], "%32ux%32u", &demoRecoder.nSrcWidth,&demoRecoder.nSrcHeight);

    sscanf(argv[6], "%32ux%32u", &demoRecoder.ndstWidth,&demoRecoder.ndstHeight);
    ...
    demoRecoder.videoConfig.nOutHeight  = demoRecoder.ndstHeight;
    demoRecoder.videoConfig.nOutWidth   = demoRecoder.ndstWidth;
    demoRecoder.videoConfig.nSrcHeight  = demoRecoder.nSrcHeight;
    demoRecoder.videoConfig.nSrcWidth   = demoRecoder.nSrcWidth;
    demoRecoder.videoConfig.nBitRate    = 3*1000*1000;
    demoRecoder.videoConfig.bUsePhyBuf  = 0;

    //AudioEncodeConfig audioConfig;
    demoRecoder.audioConfig.nType = atoi(argv[3]);
    ...
    case 1:
        ...
        else if( demoRecoder.audioConfig.nType == AUDIO_ENCODE_AAC_TYPE)
