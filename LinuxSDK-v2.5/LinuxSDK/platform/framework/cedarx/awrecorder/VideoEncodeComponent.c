...
static int setEncodeType(EncodeCompContex *p, VideoEncodeConfig* config)
{
    ...
    p->mInputHeight  = config->nSrcHeight;
    ...
    p->mInputFormat = baseConfig.eInputFormat;
...
int VideoEncodeCompInputBuffer(VideoEncodeComp* v, VideoInputBuffer *buf)
{
    ...
    if(p->mUseAllocInputBuffer)
    {
        ...
        if(p->mInputFormat == VENC_PIXEL_YUV420P || p->mInputFormat == VENC_PIXEL_YVU420P)
        {
            memcpy(inputBuffer.pAddrVirC, buf->pData+sizeY, sizeY/4);
            int nAlignW = (p->mInputWidth + 15)& ~15;
            int nAlignH = (p->mInputHeight + 15)& ~15;
            memcpy(inputBuffer.pAddrVirC+nAlignW*nAlignH/4, buf->pData+sizeY+sizeY/4, sizeY/4);
        }
        else
        {
