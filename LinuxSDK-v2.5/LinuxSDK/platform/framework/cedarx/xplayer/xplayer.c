...
static const int XPLAYER_COMMAND_SETFLOATSPEED = 0x110;
...
typedef struct PlayerContext
{
    ...
    int                 mSpeed;
    float               mFloatSpeed; //could be 0.125, 0.25, 0.5, 1, 2, 4.
    int                 mbSlowSpeedMode;
    int                 mbFast;
    int                 mFastTime;
...
void XPlayerSetFlag(XPlayer* p, int flag)
{
    ...
    PlayerSetFlag(mPriData->mPlayer, flag);
}
...
int XPlayerSetScaleDown(XPlayer* p,int bSetScaleDown)
{
    if(p != NULL)
    {
       ...
       logv("XPlayer set scale down.p->bSetScaleDown:%d",p->bSetScaleDown);
       return 0;
    }
    else
    {
...
int XPlayerPause(XPlayer* p)
{
    ...
    if(mPriData->mLivemode == 1)
    {
        ...
        logv("livemode1, get current position inmPauseTimeStamp = %" PRId64 "",
    ...
    else if(mPriData->mLivemode == 2)
    {
        ...
        logv("get current position in pause, nowUs = %ld, mPauseTimeStamp = %" PRId64 "",
...
int XPlayerSetFloatSpeed(XPlayer* p, float nSpeed)
{
    AwMessage msg;
    PlayerContext* mPriData = (PlayerContext*)p;
    mPriData->mFloatSpeed = 1.0f;
    mPriData->mSpeed = 1;

    if(nSpeed > 0 && nSpeed < 1)
    {
        mPriData->mFloatSpeed = nSpeed;
        mPriData->mbSlowSpeedMode = 1;
    }
    else
    {
        mPriData->mSpeed = (int)nSpeed;
        mPriData->mbSlowSpeedMode = 0;
    }
    if(mPriData->mSpeed == 0)
        return 0;
    //if playback changes from fast mode to slow mode, cancel seeking thread.
    if(mPriData->mbFast != 0 && nSpeed > 0 && nSpeed < 1)
    {
        mPriData->mbFast = 0;
        mPriData->mSpeed = 1;
        mPriData->mFastTime = 0;
    ...
    {
        mPriData->mbFast = 0;
        mPriData->mFloatSpeed = 1;
        mPriData->mFastTime = 0;
        PlayerPause(mPriData->mPlayer);
        PlayerSetFloatSpeed(mPriData->mPlayer, 1);
        PlayerSetDiscardAudio(mPriData->mPlayer, 0);
        int curTime;
        XPlayerGetCurrentPosition(p, &curTime);
        DemuxCompSeekTo(mPriData->mDemux, curTime+500, mPriData->mSeekModeType);
        PlayerSetDiscardAudio(mPriData->mPlayer, 0);
        XPlayerStart(p);
        return 0;
    }
    ...
    //* send a start message.
    memset(&msg, 0, sizeof(AwMessage));
    msg.messageId = XPLAYER_COMMAND_SETFLOATSPEED;
        ...
        else if(mPriData->mLivemode == 2)
        {
            ...
            if(mPriData->mStatus == XPLAYER_STATUS_PAUSED)
            {
                *msec = (tv.tv_sec * 1000000 + tv.tv_usec - mPriData->mPauseTimeStamp)/1000;
                logv("livemode2, nowUs = %ld, mPauseTimeStamp = %" PRId64 "",
                        tv.tv_sec * 1000000 + tv.tv_usec, mPriData->mPauseTimeStamp);
            }
            else
            {
                ...
                logv("-- nPositionUs = %" PRId64 "", nPositionUs);
            ...
            logv("livemode = 2, getCurrentPosition %d", *msec);
...
int getScaledownFlag(const MediaInfo *mi,int bSetScaleDown)
{
    ...
    if(bSetScaleDown == 1)
    {
        ...
        {
            logv("###source file larger than 2k,scale down.");
            ret = 1;
        }
    }
    else if(bSetScaleDown == 2)
    {
        logv("set scale down in any resolution.");
        ret = 1;
    }
    ...
    if(mi->pVideoStreamInfo->nWidth >= WIDTH_4K || mi->pVideoStreamInfo->nHeight >= HEIGHT_4K)
    {
        ...
        {
#if !defined(CONF_VP9_4K_P2P)
            ret = 1;
#endif
        }
        else if (mi->pVideoStreamInfo->eCodecFormat == VIDEO_CODEC_FORMAT_AVS2)
        {
#if !defined(CONF_AVS2_4K_P2P)
            ret = 1;
#endif
        }
...
int checkVideoSupported(const MediaInfo *mi)
{
...
#endif
#if !defined(CONF_6K)
	if(mi->pVideoStreamInfo->nWidth * mi->pVideoStreamInfo->nHeight >= PIXELS_6K)
	{
		loge("Not support 6K video !!");
		ret = -1;
	}
#endif
    ...
    {
        /* for most chips, WMV1/WMV2/VP6 specs unsupported,  play effect too bad... */
...
#if !(defined(CONF_WMV12VP6_1080P))
    else if (mi->pVideoStreamInfo->nWidth == WIDTH_1080P ||
            mi->pVideoStreamInfo->nHeight == HEIGHT_1080P)/* = 1080P */
    {
        if (mi->pVideoStreamInfo->eCodecFormat == VIDEO_CODEC_FORMAT_WMV1
                || mi->pVideoStreamInfo->eCodecFormat == VIDEO_CODEC_FORMAT_WMV2
                || mi->pVideoStreamInfo->eCodecFormat == VIDEO_CODEC_FORMAT_VP6)
    ...
    while(mPriData->mSpeedThreadRuning)
    {
        if (mPriData->mbFast == 1)
        {
            ...
            if(mPriData->mFastTime >= 0 && mPriData->mbFast == 1 )
            {
                ...
                logv("speed mode seek, mFastTime: %d, mSpeed: %d",
...
static void* XPlayerThread(void* arg)
{
    ...
    while(1)
    {
        ...
        else if(msg.messageId == XPLAYER_COMMAND_PREPARE)
        {
            ...
            if(mPriData->mStatus == XPLAYER_STATUS_PREPARED)
            {
                ...
                if(pReplySem != NULL)
                    sem_post(pReplySem);
                if(1 == msg.params[2]){
                    logv("let XPlayerPrepare return");
            ...
            msg.messageId == XPLAYER_COMMAND_SETFLOATSPEED)
        {
            ...
            //if set slower mode, just reset avtimer speed in player.
            if(msg.messageId == XPLAYER_COMMAND_SETFLOATSPEED && mPriData->mbSlowSpeedMode
                && mPriData->mFloatSpeed > 0 && mPriData->mFloatSpeed < 1)
            {
                if(mPriData->mFloatSpeed <= 0 || mPriData->mFloatSpeed > 1)
                {
                    if(pReplyValue != NULL)
                        *pReplyValue = (int)0;
                    if(pReplySem != NULL)
                        sem_post(pReplySem);
                    logw("ignored it. only support slow speed of 0.125, 0.25, 0.5.");
                    continue;
                }

                PlayerSetDiscardAudio(mPriData->mPlayer, 1);
                PlayerSetFloatSpeed(mPriData->mPlayer, mPriData->mFloatSpeed);
                if(pReplyValue != NULL)
                    *pReplyValue = (int)0;
                if(pReplySem != NULL)
                    sem_post(pReplySem);
        ...
        else if(msg.messageId == XPLAYER_COMMAND_RESET)
        {
            ...
            //* so the mStatus may be changed to PREPARED asynchronizely.
			 if(mPriData->mStatus == XPLAYER_STATUS_STOPPED)
			 {
				 logd("invalid reset() call, player in stopped status.");
                if(pReplyValue != NULL)
                    *pReplyValue = (int)0;
                if(pReplySem != NULL)
                    sem_post(pReplySem);
...
{
    PlayerContext* mPriData = (PlayerContext*)p;
    return PlayerSetDiDemo(mPriData->mPlayer, enable, left, top, right, bottom);
}

void XPlayerSetDiTnr(XPlayer* p, int enable, int mode, int strength)
{
    PlayerContext* mPriData = (PlayerContext*)p;
    return PlayerSetDiTnr(mPriData->mPlayer, enable, mode, strength);
}

void XPlayerSetDiFmd(XPlayer* p, int enable)
{
    PlayerContext* mPriData = (PlayerContext*)p;
    return PlayerSetDiFmd(mPriData->mPlayer, enable);
}
#endif
    ...
    switch(messageId)
    {
        ...
        case DEMUX_NOTIFY_EOS:
        {
            if(mPriData->mLivemode == 2) //* timeshift
            {
                ...
                logv("mCurShiftTimeStamp=%" PRId64 " ms", mPriData->mCurShiftTimeStamp/1000);
                seekTimeMs = (mPriData->mCurShiftTimeStamp - mPriData->mShiftTimeStamp)/1000
                                - mPriData->mTimeShiftDuration + mPriData->mPreSeekTimeMs;
                logv("===== seekTimeMs=%d ms, mPriData->mCurShiftTimeStamp=%" PRId64 " us,"
        ...
        {
            logv("MEDIA_INFO_BUFFERING_START,mDisplayRatio = %d",mPriData->mDisplayRatio);
        ...
        {
            logv("XPLAYER_MESSAGE_DEMUX_PAUSE_PLAYER, mStatus = %d", mPriData->mStatus);
        ...
        {
            logv("DEMUX_NOTIFY_RESET_PLAYER");
            mPriData->mLivemode = DemuxCompGetLiveMode(mPriData->mDemux);
            if(mPriData->mLivemode == 1)
            {
        ...
        {
            logv("--- hls message, DEMUX_NOTIFY_HLS_DOWNLOAD_START");
            mPriData->mCallback(mPriData->pUser, AWPLAYER_MEDIA_INFO,
        ...
        case DEMUX_NOTIFY_TIMESHIFT_DURATION:
        {
            if(mPriData->mLivemode == 2)
            {
                ...
                logv("xxxxxxxxx mPriData->mTimeShiftDuration=%" PRId64 "",
