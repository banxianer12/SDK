...
void VideoDecCompSetFlag(VideoDecComp* p, int flag)
{
    p->mFlag |= flag;
}
...
int VideoDecCompSetVideoStreamInfo(VideoDecComp* p, VideoStreamInfo* pStreamInfo, VConfig* pVconfig)
{
    ...
    logv("pStreamInfo->bSecureStreamFlag = %d",pStreamInfo->bSecureStreamFlag);
    if(pStreamInfo->bSecureStreamFlag)
    {
    ...
    int bNotUseAfbc = p->mFlag & AW_FLAG_NOT_USE_AFBC;
#if (defined(CONF_AFBC_ENABLE))
    if((pStreamInfo->eCodecFormat == VIDEO_CODEC_FORMAT_H265
        || pStreamInfo->eCodecFormat == VIDEO_CODEC_FORMAT_H264
        || pStreamInfo->eCodecFormat == VIDEO_CODEC_FORMAT_VP9
        || pStreamInfo->eCodecFormat == VIDEO_CODEC_FORMAT_AVS2)
        && bNotUseAfbc == 0)
    {
        logd("enable afbc mode for 4k");
...
int VideoDecComptStreamIndexValid(VideoDecComp* p, int nStreamIndex)
{
    if(p == NULL)
    {
        logv("no video decode component.");
        return 0;
    } else if (nStreamIndex < 0) {
        logv("nStreamIndex < 0, video stream index invalid.");
        return 0;
    }
