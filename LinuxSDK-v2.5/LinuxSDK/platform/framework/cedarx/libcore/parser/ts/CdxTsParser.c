...
#define CDX_MIN(a,b) ((a) > (b) ? (b) : (a))
#define CDX_MAX(a,b) ((a) > (b) ? (a) : (b))
#define CDX_MAX3(a, b, c) CDX_MAX(CDX_MAX(a, b), c)
...
static void UpdateTsDebugFlag(TSParser *mTSParser)
{
    #ifdef __ANDROID__
    char val[PROPERTY_VALUE_MAX];
    property_get("sys.media.video.debug", val, "0");
    if(!strcmp(val, "save-ts-stream"))
    {
        mTSParser->bSaveStreamFlag = 1;
        CDX_LOGD("save ts stream in data/camera as ts-bitstream.ts");
    }
    else
    {
        mTSParser->bSaveStreamFlag = 0;
    }
    #endif
    if(getenv("save_ts_stream"))
    {
        mTSParser->bSaveStreamFlag = 1;
        CDX_LOGD("save ts stream in data/camera as ts-bitstream.ts");
    }
...
static void PrintTsStatus(TSParser *mTSParser)
{
    ...
    CdxListForEachEntry(program, &mTSParser->mPrograms, node)
    {
        ...
        {
            CDX_LOGV("mElementaryPID = 0x%04x, "
                    "mStreamType = 0x%04x, "
                    "mMediaType = %u, "
                    "codec_id = 0x%x ",
    ...
    //类似于new_pes_av_stream
{
    ...
    codec_id   = 0xffffffff;
    //CDX_LOGD("mStream->mStreamType(0x%x)", mStream->mStreamType);
    ...
    case CDX_STREAM_TYPE_AUDIO_MPEG4:
        codec_type = TYPE_AUDIO;
        codec_id   = AUDIO_CODEC_FORMAT_MPEG_AAC_LATM;
        break;
    ...
    case CDX_STREAM_TYPE_HDMV_PGS_SUBTITLE:
        ...
        break;
    case CDX_STREAM_TYPE_AUDIO_DRA:
        codec_type = TYPE_AUDIO;
        codec_id   = AUDIO_CODEC_FORMAT_DRA;
        break;
    case CDX_STREAM_TYPE_VIDEO_AVS:
        codec_type = TYPE_VIDEO;
        codec_id   = VIDEO_CODEC_FORMAT_AVS;
        break;
    case CDX_STREAM_TYPE_VIDEO_AVS2:
        codec_type = TYPE_VIDEO;
        codec_id   = VIDEO_CODEC_FORMAT_AVS2;
        break;
    ...
    */
    switch(descriptor_tag)
    {
        case 0x0a:/*ISO_639_language_descriptor*/
            ...
            {
                CDX_LOGW("tag %x would be ignored since invalid descriptor length %u,",
        ...
        case 0x56: /* DVB teletext descriptor */
            if ((descriptor_length == 0) || (descriptor_length > 0 && descriptor_length % 5 != 0))
            {
                CDX_LOGW("tag 0x%02x would be ignored since invalid descriptor length %u,",
                    descriptor_tag, descriptor_length);
                break;
            }
            stream = (Stream *)father;
        ...
        case 0x59:/*DVB subtitle*/
            if ((descriptor_length == 0) || (descriptor_length > 0 && descriptor_length % 8 != 0))
            {
                CDX_LOGW("tag 0x%02x would be ignored since invalid descriptor length %u,",
                    descriptor_tag, descriptor_length);
                break;
        ...
        case 0x81: /* AC-3 audio descriptor */
            ...
            metadata = (AudioMetaData *)stream->metadata;
        ...
        case 0x05:
            ...
            else if(format_identifier == MKBETAG('D', 'R', 'A', '1'))
            {
                stream = (Stream *)father;
                stream->mStreamType = CDX_STREAM_TYPE_AUDIO_DRA;
                stream->mMediaType = TYPE_AUDIO;
            }
            else
            {
        ...
        case 0xA0:
            stream = (Stream *)father;
            if(stream->mStreamType != CDX_STREAM_TYPE_AUDIO_DRA)
            {
                CDX_LOGW("not dra");
                break;
            }
            ...
            unsigned sample_rate_index = CdxBitReaderGetBits(br, 4);
            metadata->samplingFrequency = DRA_SamplingRate[sample_rate_index];

            unsigned num_normal_channels = CdxBitReaderGetBits(br, 6);
            metadata->channelNum = num_normal_channels; //spec defines "+1"

            unsigned num_lfe_channels = CdxBitReaderGetBits(br, 2);
            CDX_LOGV("num_lfe_channels:%d, num_normal_channels:%d", num_lfe_channels, num_normal_channels);
            if(num_lfe_channels>3)
            {
                CDX_LOGW("dra channel error!!!");
            }
            unsigned dra_version_flag = CdxBitReaderGetBits(br, 1);
            CDX_CHECK(dra_version_flag == 0);

            unsigned text_present_flag = CdxBitReaderGetBits(br, 1);
            unsigned language_present_flag = CdxBitReaderGetBits(br, 1);
            CdxBitReaderGetBits(br, 1);//reserved
            descriptor_length -= 2;
            if(text_present_flag)
            {
                unsigned text_length = CdxBitReaderGetBits(br, 8);
                CdxBitReaderSkipBits(br, text_length*16);//text
                descriptor_length -= text_length*2;
            }
            if(language_present_flag)
            {
                CdxBitReaderSkipBits(br, 24);//language
                descriptor_length -= 3;
            }
            //CdxBitReaderSkipBits();//additional_info
            break;
        default:
...
status_t parseProgramMap(Program *program, CdxBitReaderT *br)
{
    ...
    if(version_number <  program->version_number && program->version_number != (unsigned)-1)
    {
    ...
    cdx_int32 packet_size, cdx_int32* index)
{
    ...
    for(j=i=0; i<size; i++)
    {
        if(buf[i] == 0x47)
        {
            if((buf[i+3] & 0x30) || (U16_AT(buf+1) & 0x1FFF) == 0X1FFF)
            {
                stat[j]++;
                stat_all++;
                if(stat[j] > result)
    ...
    CDX_LOGV("result: %d. result2: %d.", result,
        ((stat_all - 10 * result)>0?(stat_all - 10 * result):0)/10);
    return result - CDX_MAX((stat_all - 10 * result), 0)/10;
}
...
static cdx_int32 TSResync(TSParser *mTSParser)
{
        ...
        && mCacheBuffer->validDataSize < resyncSize)
    {
        ...
        if(mTSParser->bSaveStreamFlag)
        {
            if(ret > 0)
            {
                if (mTSParser->pFile)
                {
                    fwrite(mCacheBuffer->bigBuf + mCacheBuffer->writePos, 1, ret,
                                    mTSParser->pFile);
                    sync();
                }
                else
...
cdx_int32 TSClose(CdxParserT *parser)
{
    ...
    if (mTSParser->pFile && mTSParser->bSaveStreamFlag)
    {
        fclose(mTSParser->pFile);
        mTSParser->pFile = NULL;
    }
...
#if 0
static cdx_int32 VerifyVideoStream(TSParser *mTSParser, CdxMediaInfoT *pmediainfo)
{
    VideoStreamInfo *video = &pmediainfo->program[0].video[0];
    if(pmediainfo->program[0].videoNum == 1 && video->nWidth == 0 &&
        ...
        !mTSParser->tempVideoInfo.nCodecSpecificDataLen)
    {
        ...
        CdxListForEachEntrySafe(stream, nextstream, &mTSParser->curProgram->mStreams, node)
        {
            if(stream->mMediaType == TYPE_VIDEO)
                DestroyStream(stream);
        }
...
#endif
/*
static cdx_int32 VerifyAudioStream(TSParser *mTSParser, CdxMediaInfoT *pmediainfo)
{
    AudioStreamInfo *audio = &pmediainfo->program[0].audio[0];
    if(pmediainfo->program[0].audioNum == 1 && audio->nChannelNum == 0 &&
        audio->nSampleRate == 0 && audio->nBitsPerSample ==0)
    {
        pmediainfo->program[0].audioNum = 0;
        mTSParser->curProgram->audioCount = 0;
        if(audio->eCodecFormat)
        {
...
static cdx_int32 SetMediaInfo(TSParser *mTSParser, CdxMediaInfoT *pmediainfo)
{
    ...
    //some ts stream contains none video/audio stream data but its PMT has stream info, re-check it.
    //now I could only deal with the case that only one video stream and one audio stream are contained.
    //TODO:
    //VerifyVideoStream(mTSParser, pmediainfo);
    //VerifyAudioStream(mTSParser, pmediainfo);
    return 0;
}
...
#endif
    //If All cacheBuffer data is firstly under limited size, remalloc the cache size.
    cdx_uint8* tmpBuf;
    CacheBuffer* mCacheBuffer = &mTSParser->mCacheBuffer;
    if(mCacheBuffer->validDataSize < CACHE_BUFFER_SIZE  && !mTSParser->bResetCacheBufferSizeFlag)
    {
        tmpBuf = (cdx_uint8*)CdxMalloc(CACHE_BUFFER_SIZE);
        if(tmpBuf)
        {
            CDX_LOGV("reset cache.readPos = %d, endPos = %d, validDataSize = %d",
                mCacheBuffer->readPos, mCacheBuffer->endPos, mCacheBuffer->validDataSize);
            memcpy(tmpBuf, mCacheBuffer->bigBuf + mCacheBuffer->readPos, mCacheBuffer->validDataSize);
            free(mCacheBuffer->bigBuf);
            mCacheBuffer->bigBuf = (cdx_uint8*)CdxMalloc(CACHE_BUFFER_SIZE);
            if(!mCacheBuffer->bigBuf)
            {
                loge("wtf, fail to malloc cache buffer of %d kB.", CACHE_BUFFER_SIZE / 1024);
                abort();
            }
            memcpy(mCacheBuffer->bigBuf, tmpBuf, mCacheBuffer->validDataSize);
            mCacheBuffer->readPos = 0;
            mCacheBuffer->endPos = -1;
            mCacheBuffer->writePos = mCacheBuffer->validDataSize;
            mCacheBuffer->bufSize = CACHE_BUFFER_SIZE;
            if(mCacheBuffer->bufSize - mCacheBuffer->validDataSize <
                            (cdx_uint32)mTSParser->sizeToReadEverytime){
                mCacheBuffer->endPos = mCacheBuffer->writePos;
            }
            free(tmpBuf);
        ...
        {
            logw("fail to reset cache buffer size from %d kB to %d kB.",
                mCacheBuffer->bufSize / 1024, CACHE_BUFFER_SIZE / 1024);
        }
        mTSParser->bResetCacheBufferSizeFlag = 1;
    }
...
static cdx_int32 SelectOrUpdateProgram(TSParser *mTSParser)
{
    cdx_int32 ret = 0;
...
_GetES:
    while(!mTSParser->currentES)
    {
        ...
        ret = SelectOrUpdateProgram(mTSParser);
        if(ret== -1)
            goto _exit;
        else if(ret == 1)
            continue;
...
cdx_int32 SeekToTime(TSParser *mTSParser, cdx_int64 timeUs, cdx_uint32 mode)
{
    ...
    if(mTSParser->seekMethod == 0)
    {
        ...
        do
        {
            ...
            if (pts == (cdx_int64)-2)//forcestop的情况
            {
                break;
            }
            else if (pts == -1)
            {
                if (tryTimes++ < 100) // try 204*60*100 bytes size
                {
                    logv("tryTimes=%d", tryTimes);
                    continue;
                }
                else
                {
            ...
            CDX_LOGV("pts = %lld, pts_diff = %lld, cnt = %u, mTSParser->byteRate=%llu", pts/90000, pts_diff/90000, cnt,
                mTSParser->byteRate);
            if (pts > dst_pts + pts_high_thredhold)
            {
                file_pos -= mTSParser->byteRate*(pts_diff/90000) / 10;
                if (file_pos <= 0)
                    file_pos = 0;
            }
            ...
            {
                file_pos += mTSParser->byteRate*(pts_diff/90000) / 10;
                while (file_pos > mTSParser->fileSize)
                {
                    file_pos -= mTSParser->byteRate*(pts_diff/90000) / 10;
                }
            }
...
cdx_uint32 TSParserProbe(CdxStreamProbeDataT *probeData)
{
    ...
    {
        cdx_int32 maxscore = 0;
        cdx_int32 sumscore = 0;
        cdx_int32 i = 0;
        cdx_int32 check_count = (cdx_int32)probeData->len / TS_FEC_PACKET_SIZE;
        #define CHECK_COUNT 10
        #define CHECK_BLOCK 100
        cdx_uint8* buf = (cdx_uint8*)probeData->buf;
        if(!check_count)
            return -1;
        for(i = 0; i < check_count; i += CHECK_BLOCK)
        {
            cdx_int32 left = CDX_MIN((check_count- i), CHECK_BLOCK);
            score = Analyze(buf + TS_PACKET_SIZE * i, TS_PACKET_SIZE * left, TS_PACKET_SIZE, NULL);
            dvhs_score = Analyze(buf + TS_DVHS_PACKET_SIZE * i, TS_DVHS_PACKET_SIZE * left,
                                    TS_DVHS_PACKET_SIZE, NULL);
            fec_score = Analyze(buf + TS_FEC_PACKET_SIZE * i, TS_FEC_PACKET_SIZE * left,
                                    TS_FEC_PACKET_SIZE, NULL);
            score = CDX_MAX3(score, dvhs_score, fec_score);
            sumscore += score;
            maxscore = CDX_MAX(maxscore, score);
        }
        sumscore = sumscore * CHECK_COUNT / check_count;
        maxscore = maxscore * CHECK_COUNT / CHECK_BLOCK;
        CDX_LOGV("TS score: sumscore [%d] maxscore [%d].", sumscore, maxscore);
        if(check_count > CHECK_COUNT && sumscore > 6)
            return 100;//100 + sumscore - CHECK_COUNT;
        else if(check_count >= CHECK_COUNT && sumscore > 6)
            return 100;//50 + sumscore - CHECK_COUNT;
    ...
    {
        CDX_LOGV("Probe data is not enough. probeData->len(%u)", probeData->len);
...
CdxParserT *TSParserOpen(CdxStreamT *cdxStream, cdx_uint32 flags)
{
    ...
    UpdateTsDebugFlag(mTSParser);
    if(mTSParser->bSaveStreamFlag){
        mTSParser->pFile = fopen("/data/camera/ts-bitstream.ts", "wb+");
        if (!mTSParser->pFile)
        {
