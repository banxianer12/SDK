...
__attribute__((section(".data"))) axp_contrl_info pmu_axp81X_ctrl_tbl[] = {
	...
	{ "dldo2", 700, 3300, AXP81X_DLDO2_VOL, 0x1f, 100, 0, 0,
	  AXP81X_OUTPUT_CTL2, 4 },
	{ "dldo3", 700, 3300, AXP81X_DLDO3_VOL, 0x1f, 100, 0, 0,
	  AXP81X_OUTPUT_CTL2, 5 },
	{ "dldo4", 500, 3300, AXP81X_DLDO4_VOL, 0x1f, 100, 0, 0,
	  AXP81X_OUTPUT_CTL2, 6 },
	{ "dc1sw", 0, 3300, AXP81X_DLDO4_VOL, 0x1f, 100, 0, 0,
...
{
	if (pmic_bus_write(AXP81X_RUNTIME_ADDR, AXP81X_DATA_BUFFER3,
			   (u8)status)) {
		return -1;
	}
	return 0;
...
{
	u8 reg_value;
	if (pmic_bus_read(AXP81X_RUNTIME_ADDR, AXP81X_DATA_BUFFER3, &reg_value)) {
		return -1;
	}
	return reg_value;
}
...
{
	u8 reg_value;
	if (pmic_bus_read(AXP81X_RUNTIME_ADDR, AXP81X_INTSTS1, &reg_value)) {
		return -1;
	}
	reg_value &= (0x03 << 2);
	if (reg_value) {
		if (pmic_bus_write(AXP81X_RUNTIME_ADDR, AXP81X_INTSTS1,
				   reg_value)) {
			return -1;
		}
	}
