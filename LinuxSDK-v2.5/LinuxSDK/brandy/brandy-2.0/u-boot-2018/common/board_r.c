...
static int auto_update_system(void)
{
	...
	ret = script_parser_fetch("/soc/target", "auto_usb_ota", &auto_usb_ota, 0);
	if (ret < 0) {
		printf("get auto_usb_ota failed!\n");
		return 0;
	}

	printf("work_mode:%d auto_usb_ota:%d\n", work_mode, auto_usb_ota);
	if ((work_mode == WORK_MODE_BOOT) && (1 == auto_usb_ota))
...
#define PE_CFG0_ADDR 0x0300b090
#define PE_DAT_ADDR  0x0300b0a0
...
void board_init_r(gd_t *new_gd, ulong dest_addr)
{
	...
	reg_val = readl(PE_CFG0_ADDR);
	reg_val &= ~(0xf << 4);
	reg_val |= (0x1 << 4);
	writel(reg_val, PE_CFG0_ADDR);
	/* Set led2 gpio output high */
	reg_val = readl(PE_DAT_ADDR);
	reg_val |= (0x1 << 1);
	writel(reg_val, PE_DAT_ADDR);
